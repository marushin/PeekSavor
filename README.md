...
### 2단계: API 프로바이더 설정
1. 좌측 사이드바에서 **"API Settings"** 버튼 클릭
2. 우측에 설정 패널이 나타납니다
3. 사용할 프로바이더 타입 선택 (예: Google, LMStudio)
4. 프로바이더 이름과 API Key/URL 입력
5. **"Verify"** 버튼으로 연결 확인
6. **"Add"** 버튼으로 프로바이더 추가
...
2. **명령줄**: `PeekSavor.exe "이미지경로.jpg"` 로 실행
